...
The saved model has not been retrained on this new real data, and the reason is worth stating plainly rather than calling it a future step. The audit study's own researchers designed it to hold resume quality roughly constant and randomize one thing: a name signaling race and gender, so they could isolate its effect on callbacks. That means whether someone got a callback in this data was driven substantially by a signal this classifier deliberately never sees, since protected attributes are excluded from matching on purpose. Training a model built on semantic similarity, skill overlap, experience, and education against outcomes mostly explained by something outside those inputs would not teach it what makes a good match, it would fit whatever weak, coincidental patterns happen to exist between the features and an outcome that is largely noise as far as those features are concerned.

There is a second reason on top of that: every one of the 4,870 real examples comes from one narrow slice, entry level clerical and sales jobs, in two U.S. cities, over twenty years ago. Training on that and applying the result to score a senior engineering role or a nurse practitioner role today would mean trusting a model far outside anything it was ever validated on. Not deploying a model past the range it was actually tested on is standard practice, and right now that range is one narrow study, not a broad picture of real hiring.
